...
"1")
    ...
    java -jar ./jmeter/bin/ApacheJMeter.jar -n -f -t ./tests/google.jmx -l ./results/result-$REPORT_TIME.jtl -e -o ./report/$REPORT_TIME
    ;;
"2")
    echo "Running JMeter scenario with Variant 2: , Parameter: $PARAMETER_VALUE"
    java -jar ./jmeter/bin/ApacheJMeter.jar -n -f -t -f ./tests/duck-go-go.jmx -l ./results/result-$REPORT_TIME.jtl -e -o ./report/$REPORT_TIME
    ;;
*)
    echo "Invalid variant. Please specify one of: 1, 2 where 1 is Google, 2 is DuckGoGO"
    exit 1
    ;;
esac